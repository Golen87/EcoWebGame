...
export class ScrollArea extends Phaser.GameObjects.Container {
	...
	constructor(scene: BaseScene, x: number, y: number, width: number, height: number, fadePadding: number) {
		super(scene, x, y);
		this.scene = scene;
		this.width = width;
		this.height = height;
		...
		this.area.on('dragstart', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
			this.hold = true;
			grabY = -this.area.y + this.content.y;
		});
		this.area.on('drag', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
			let newTargetY = dragY + grabY;
			this.speedY = (newTargetY - this.targetY);
			this.targetY = newTargetY;
		});
		this.area.on('dragend', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
			this.hold = false;
		});
		...
		this.maskArea = new Phaser.Display.Masks.GeometryMask(this.scene, this.maskGraphics);
	...
	update(time: number, delta: number) {
		...
		if (this.targetY < -this.contentHeight+this.height+0*this.fadePadding) {
			this.targetY = -this.contentHeight+this.height+0*this.fadePadding;
		}

		// Smooth approach
		this.content.y += (this.targetY - this.content.y) * (1 - Math.pow(0.5, 60*delta/1000));
	}
	...
	apply(gameObject: any) {
		...
		this.contentHeight = Math.max(this.contentHeight, gameObject.y + gameObject.height);
	}
